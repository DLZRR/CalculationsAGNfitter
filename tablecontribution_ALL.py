...
array = np.zeros((len(OTHER_BANDS), 68))
...
for i in range(0, len(OTHER_BANDS), 1): 
    ...
    #print OTHER_BANDS[i][0]
        if os.path.isfile(folder + '/parameter_outvalues_' + str(OTHER_BANDS[i][0]) + '.txt'):

            my_data = np.loadtxt(folder + '/parameter_outvalues_' + str(OTHER_BANDS[i][0]) + '.txt', skiprows = 4)
...
for i in range(0, len(OTHER_BANDS), 1): 
    ...
    if os.path.isfile(folder + '/parameter_outvalues_' + str(OTHER_BANDS[i][0]) + '.txt'):
        ...
        if my_data[2][20] > -1000.:#np.mean(ln_likelihood) - 1.96*np.std(ln_likelihood):
            ...
            array[t][66] = OTHER_BANDS[i][25]
            array[t][67] = OTHER_BANDS[i][26]
